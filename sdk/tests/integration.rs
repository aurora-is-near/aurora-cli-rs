...
async fn test_view_call_sandbox() -> anyhow::Result<()> {
    ...
    let workspace = Workspace::new(worker.rpc_addr().as_str(), None, signer_from_secret(root.id(), root.secret_key()))?;
    ...
    let spec = metadata_val["spec"].as_str().ok_or_else(|| anyhow::anyhow!("Missing or invalid 'spec' in metadata"))?;
    let name = metadata_val["name"].as_str().ok_or_else(|| anyhow::anyhow!("Missing or invalid 'name' in metadata"))?;
    let symbol = metadata_val["symbol"].as_str().ok_or_else(|| anyhow::anyhow!("Missing or invalid 'symbol' in metadata"))?;
    let decimals = metadata_val["decimals"].as_u64().ok_or_else(|| anyhow::anyhow!("Missing or invalid 'decimals' in metadata"))? as u8;
...
async fn test_transaction_sandbox() -> anyhow::Result<()> {
    ...
    let workspace = Workspace::new(worker.rpc_addr().as_str(), None, signer_from_secret(owner_account.id(), owner_account.secret_key()))?;
    ...
    let min_bound_str = bounds_val["min"].as_str()
        .ok_or_else(|| anyhow::anyhow!("Missing or invalid 'min' in storage_balance_bounds response"))?;
    // Parse the min bound string to u128, then create NearToken
    let min_bound_u128 = min_bound_str.parse::<u128>()
        .map_err(|e| anyhow::anyhow!("Failed to parse min bound string '{}' to u128: {}", min_bound_str, e))?;
    ...
    let owner_balance_u128 = owner_balance_str.parse::<u128>()
         .map_err(|e| anyhow::anyhow!("Failed to parse owner balance string '{}' to u128: {}", owner_balance_str, e))?;
    ...
    let receiver_balance_u128 = receiver_balance_str.parse::<u128>()
         .map_err(|e| anyhow::anyhow!("Failed to parse receiver balance string '{}' to u128: {}", receiver_balance_str, e))?;
...
async fn test_ft_balance_of_sandbox() -> anyhow::Result<()> {
    ...
    let balance_u128 = balance_str.parse::<u128>()
        .map_err(|e| anyhow::anyhow!("Failed to parse balance string '{}' to u128: {}", balance_str, e))?;
    ...
    let balance_other_u128 = balance_other_str.parse::<u128>()
        .map_err(|e| anyhow::anyhow!("Failed to parse other balance string '{}' to u128: {}", balance_other_str, e))?;
...
async fn test_view_access_keys_sandbox() -> anyhow::Result<()> {
    ...
    let access_key_list: AccessKeyList = workspace
        .view_access_keys(owner_account.id()).await?;
    ...
    println!("Successfully verified access key for: {}", owner_account.id());
...
async fn test_batch_transaction_sandbox() -> anyhow::Result<()> {
    ...
    let bounds_result = workspace
        ...
        .await?;
    let bounds_val: serde_json::Value = serde_json::from_slice(&bounds_result.result)?;
    let min_bound_str = bounds_val["min"].as_str().ok_or_else(|| anyhow::anyhow!("Missing min bound"))?;
    let min_bound_u128 = min_bound_str.parse::<u128>()?;
    let minimum_deposit = NearToken::from_yoctonear(min_bound_u128);
    ...
    assert_eq!(receiver_balance, transfer_amount, "Receiver balance should match transferred amount after batch");

    Ok(())
}

fn signer_from_secret(account_id: &near_workspaces::AccountId, sk: &near_workspaces::types::SecretKey) -> near_crypto::InMemorySigner {
